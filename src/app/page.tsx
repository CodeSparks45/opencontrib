      ...
      pts.forEach((p, i) => {
        ...
        pts.slice(i + 1).forEach(q => {
          ...
          if (d < 130) {
            ...
            ctx.strokeStyle = `rgba(0,229,255,${0.05 * (1 - d / 130)})`;
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        :root{
          ...
          --cyan:#00E5FF;--cdim:rgba(0,229,255,.1);--cglow:rgba(0,229,255,.3);
        ...
        /* grid bg */
        .lp-wrap{position:relative;min-height:100vh;overflow:hidden;}
        .lp-grid{position:fixed;inset:0;background-image:linear-gradient(rgba(0,229,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(0,229,255,.025) 1px,transparent 1px);background-size:56px 56px;pointer-events:none;z-index:0;}
        .lp-orb{position:fixed;border-radius:50%;pointer-events:none;z-index:0;}
        .lp-o1{width:700px;height:700px;background:radial-gradient(circle,rgba(0,229,255,.07) 0%,transparent 70%);top:-15%;left:-10%;animation:lp-drift1 18s ease-in-out infinite;}
        .lp-o2{width:600px;height:600px;background:radial-gradient(circle,rgba(124,58,237,.09) 0%,transparent 70%);top:25%;right:-8%;animation:lp-drift2 22s ease-in-out infinite;}
        .lp-o3{width:450px;height:450px;background:radial-gradient(circle,rgba(0,255,163,.06) 0%,transparent 70%);bottom:5%;left:20%;animation:lp-drift3 16s ease-in-out infinite;}
        @keyframes lp-drift1{0%,100%{transform:translate(0,0)}33%{transform:translate(50px,-35px)}66%{transform:translate(-25px,55px)}}
        @keyframes lp-drift2{0%,100%{transform:translate(0,0)}33%{transform:translate(-60px,45px)}66%{transform:translate(35px,-25px)}}
        @keyframes lp-drift3{0%,100%{transform:translate(0,0)}50%{transform:translate(-40px,-35px)}}

        /* nav */
        .lp-nav{position:sticky;top:0;z-index:10000;backdrop-filter:blur(24px);background:rgba(5,8,16,.75);border-bottom:1px solid rgba(255,255,255,.05);}
        ...
        /* buttons - FORCED Z-INDEX AND POINTER EVENTS TO OVERRIDE GLOWS */
        .lp-btn{position:relative;z-index:99999 !important;pointer-events:auto !important;display:inline-flex;align-items:center;gap:7px;padding:10px 22px;border-radius:12px;font-family:var(--fd);font-weight:700;font-size:13.5px;cursor:pointer;border:none;transition:all .25s cubic-bezier(.16,1,.3,1);overflow:hidden;text-decoration:none;white-space:nowrap;}
        .lp-btn-p{background:linear-gradient(135deg,#00C8EC,#0077FF);color:#000;box-shadow:0 0 0 transparent;}
        ...
        .lp-h1{font-family:var(--fd);font-size:clamp(42px,5.2vw,70px);font-weight:800;line-height:1.03;letter-spacing:-.035em;margin-bottom:22px;}
        ...
        .lp-dim{color:var(--t3);}
        .lp-sub{color:var(--t2);font-size:16px;line-height:1.78;max-width:480px;margin-bottom:34px;opacity:0;animation:lp-fadeUp .6s .5s forwards;position:relative;z-index:10;}
        
        /* 🚨 HIGHEST Z-INDEX FOR LEFT SIDE CONTENT TO AVOID GLOW OVERLAP 🚨 */
        .lp-left-content { position: relative; z-index: 1000 !important; }
        
        .lp-acts{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:48px;opacity:0;animation:lp-fadeUp .6s .65s forwards;position:relative;z-index:1000 !important;pointer-events:auto;}
        ...
        /* terminal card */
        .lp-vis{position:relative;opacity:0;animation:lp-fadeUp 1s .3s forwards; z-index: 1;}
        .lp-float{position:absolute;backdrop-filter:blur(20px);background:rgba(8,13,26,.92);border:1px solid var(--bd);border-radius:12px;padding:10px 16px;box-shadow:0 8px 32px rgba(0,0,0,.5);z-index:5;}
        ...
        .lp-bc{background:var(--bg1);border:1px solid var(--bd);border-radius:22px;padding:26px;overflow:hidden;position:relative;transition:transform .4s cubic-bezier(.16,1,.3,1),border-color .3s,box-shadow .3s;will-change:transform;}
        .lp-bc::after{content:'';position:absolute;inset:0;border-radius:22px;background:linear-gradient(135deg,rgba(255,255,255,.035) 0%,transparent 55%);pointer-events:none;}
        ...
        /* testimonials */
        .lp-tcards{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-top:52px;}
        ...
        .lp-cta-glow{position:absolute;width:500px;height:400px;border-radius:50%;background:radial-gradient(ellipse,rgba(0,229,255,.07) 0%,transparent 70%);top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none;}
        ...
        .lp-d1{transition-delay:.1s}.lp-d2{transition-delay:.2s}.lp-d3{transition-delay:.3s}.lp-d4{transition-delay:.4s}.lp-d5{transition-delay:.5s}
      `}} />

      <div className="lp-wrap">
        <canvas ref={canvasRef} style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, opacity: 0.55 }} />
        ...
        {/* ── MARQUEE ── */}
        <div className="lp-marquee" style={{ position: "relative", zIndex: 10 }}>
          <div className="lp-mtrack">
            {["React","Rust","Python","TypeScript","Go","Kubernetes","Next.js","Svelte","WebAssembly","PostgreSQL","Redis","Vue","Angular","Docker","GraphQL","Node.js"].map(t => (
        ...
        <section className="lp-sec" id="lp-features">
          ...
          <div className="lp-bento lp-rv lp-d1">
            ...
            <div className="lp-bc lp-b-wide" style={{ background: "linear-gradient(145deg,rgba(0,229,255,.04),var(--bg1))" }}>
              ...
              <div className="lp-cdesc">Claude AI analyzes your GitHub history, tech stack, and expertise to surface the perfect issues every single time. No more scrolling through hundreds of irrelevant tickets.</div>
              <div style={{ marginTop: "18px" }}>
                <div className="lp-drow"><div className="lp-dlbl">Beginner</div><div className="lp-dbarw"><div className="lp-dbar lp-db" data-w="72%" id="db1"/></div><div className="lp-dcnt">72%</div></div>
                <div className="lp-drow"><div className="lp-dlbl">Intermediate</div><div className="lp-dbarw"><div className="lp-dbar lp-di" data-w="55%" id="db2"/></div><div className="lp-dcnt">55%</div></div>
                <div className="lp-drow"><div className="lp-dlbl">Advanced</div><div className="lp-dbarw"><div className="lp-dbar lp-da" data-w="28%" id="db3"/></div><div className="lp-dcnt">28%</div></div>
              </div>
            </div>
            {/* Streak card */}
            <div className="lp-bc" style={{ background: "linear-gradient(145deg,rgba(251,146,60,.04),var(--bg1))" }}>
              ...
              <div className="lp-cdesc">Daily contribution goals keep you consistent. Miss a day and your streak resets — the pressure is the point.</div>
            ...
            <div className="lp-bc" style={{ background: "linear-gradient(145deg,rgba(0,255,163,.035),var(--bg1))" }}>
              ...
              <div className="lp-cdesc">Watch contributors merge PRs in real-time. Let the momentum push you forward.</div>
            ...
            <div className="lp-bc" style={{ background: "linear-gradient(145deg,rgba(251,191,36,.04),var(--bg1))" }}>
              ...
              <div className="lp-cdesc">Earn XP for every merged PR. Unlock badges, climb the leaderboard.</div>
              <div style={{ marginTop: "14px", fontFamily: "var(--fd)", fontSize: "11px", fontWeight: 700, color: "var(--t3)", marginBottom: "6px" }}>LEVEL 12 → 13</div>
            ...
            <div className="lp-bc" style={{ background: "linear-gradient(145deg,rgba(255,45,120,.04),var(--bg1))" }}>
              ...
              <div className="lp-cdesc">Tinder-style issue swiping. Save what interests you, skip what doesn't. Fast and fun.</div>
            </div>
          </div>
        </section>
        ...
        {/* ── TESTIMONIALS ── */}
        <section className="lp-sec" id="lp-testimonials" style={{ paddingTop: "0" }}>
          ...
          <div className="lp-tcards lp-rv lp-d1">
            {[
              { av:"AK", color:"#00E5FF", q:"\"", txt:"Found my first open-source contribution within 10 minutes. The AI matching is scary accurate — it knew exactly what I could handle.", name:"Alok Kumar", role:"Final year CSE, NIT Surat" },
              { av:"RV", color:"#7C3AED", q:"\"", txt:"The streak system got me contributing daily for 3 weeks straight. I've merged 11 PRs. My GitHub looks amazing now.", name:"Riya Verma", role:"Frontend Dev, Bangalore" },
              { av:"SG", color:"#00FFA3", q:"\"", txt:"Swipe mode is addictive. I went through 50 issues in like 15 minutes and saved 8 that matched my React skills perfectly.", name:"Sahil Gupta", role:"GSSoC 2024 Contributor" },
            ].map(t => (
              <div key={t.name} className="lp-tcard">
                <div className="lp-stars">{[...Array(5)].map((_,i)=><span key={i} className="lp-star">★</span>)}</div>
                <div className="lp-tq">{t.q}</div>
